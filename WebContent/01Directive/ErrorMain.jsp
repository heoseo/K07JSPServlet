...
<%--
	* errorPage 지시어
		: 해당 페이지에서 에러(예외)가 발생되었을 떄 예외처리를 설정한 페이지에서 하겠다는 선언.
		: 500에러와 같은 페이지를 사용자에게 보여주지 않으므로 사이트에 대한 신뢰도를 높일 수 있다.
 --%>
<%
...
/* 
해당 페이지로 폼값을 전송할 떄 "나이" 부분을 공백상태 혹은 숫자가 아닌 일부를
입력 후 전송하면 해당 코드에서 에러가 발생한다.*/
if(strAge != null){
	//integar.persint()는 숫자가 아닌경우 예외 발생함.
